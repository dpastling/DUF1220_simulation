...
result_folder=alignments/spikein/total

#DOMAINS=( `cat remaining_domains.txt`)
...
do
	...
	gene=`echo $sample | perl -pe 's/^(NBPF[^_]+).+?$/$1/'`
        ...
        first_pair=$fastq_folder/$gene/${sample}_1.fastq
        second_pair=$fastq_folder/$gene/${sample}_2.fastq

	gunzip -c $first_pair.gz > $first_pair
	gunzip -c $second_pair.gz > $second_pair
	
	code/bowtie_total.sh -g $genome -b $result_folder/$gene/${sample}_paired.bam $first_pair $second_pair

	code/bowtie_total.sh -g $genome -b $result_folder/$gene/${sample}_single.bam $first_pair
